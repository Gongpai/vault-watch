...
- เพิ่ม `fuzz/Cargo.lock` สำหรับ reproduce nightly fuzz build โดยไม่เปลี่ยน production dependency lock
- อัปเดต Sprint 10E ให้ bounded ATA fuzz campaign ผ่านแล้ว; curated synthetic seeds และ hardware qualification ยังเหลืออยู่

### Validated

- `ata_pages`: 14,898,840 executions/61s, `cov 470`, `ft 853`, peak RSS 501 MiB
- `ata_return_descriptor`: 60,864,910 executions/61s, `cov 56`, `ft 80`, peak RSS 502 MiB
- ทั้งสอง target จบด้วย `DONE`; ไม่มี ASan finding, crash, timeout หรือไฟล์ใน `fuzz/artifacts/`

### Security

- corpus ที่ fuzzer สร้างยังถูก ignore และไม่ถูก commit
- validation ใช้ arbitrary generated bytes เท่านั้น ไม่มี real IDENTIFY, SMART, serial หรือ WWN capture
