...
**Version:** 0.5.0 | **Duration:** 2026-08-05 → 2026-08-19 | **Status:** ✅ Done
...
- [x] `make build` ผ่านไม่มี error ใหม่
- [x] `cargo test` ผ่านทั้งหมด
- [x] ย้าย hardware topology verification ไป [US-MON-38](../user-stories/US-MON-38.md)
- [x] Config override `devices = [...]` ทำงานถูกต้อง
- [x] ย้าย no-device/partial inventory ไป [US-MON-29](../user-stories/US-MON-29.md) และ UI ไป US-MON-32
- [x] `contrib/config.example.toml` มี `devices` option

**US-MON-19:**
- [x] Key hint bar แสดงที่ด้านล่างสุดตลอดเวลา (Table + Graph view)
- [x] Key label มี invert background (nano-style)
- [x] Shortcuts เปลี่ยนตาม view mode ปัจจุบัน
- [x] ไม่ล้น terminal แคบ
- [x] Header ไม่มี shortcut ซ้ำซ้อนแล้ว
